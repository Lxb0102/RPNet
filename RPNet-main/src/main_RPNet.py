...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='RPNet', help="model name")
    ...
    parser.add_argument('-p', '--pretrain_prefix', type=str, default='', help='log dir prefix like "log0", for finetune')
    ...
    # parser.add_argument('--weight_multi', type=float, default=0.03, help='weight of multilabel_margin_loss')
...
class PairwiseContrastiveLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        ...
        if self.mode == 'triplet':
            ...
            exp_neg = torch.exp(neg_sim / self.temperature)
            loss = -torch.log(exp_pos / (exp_pos + exp_neg)).mean()
        else:
            raise ValueError(f"Unsupported mode: {self.mode}")

        return loss  # 直接返回标量损失值
...
def evaluator(args, model, data_val, voc_size, epoch, 
    ...
    for patient in tqdm(data_val, ncols=60, total=len(data_val), desc=f"Evaluation"):
        ...
        patient, useful_visits = recommend_medication(patient, data_val, sample_rate=10,
...
def evaluator_mask(model, data_val, voc_size, epoch, device, mode='pretrain'):
    ...
    for batch in tqdm(data_val, ncols=60, desc=mode, total=len_val):
        ...
        batch, useful_visits = recommend_medication(batch, data_val, sample_rate=10,
                                                      similarity_threshold=0.5)
        result = model(batch, useful_visits, mode)
...
def evaluator_nsp(model, data_val, data, epoch, device, mode='pretrain_nsp'):
    ...
    for batch_data in tqdm(data_val, ncols=60, desc=mode, total=len(data_val)):
        ...
        nsp_batch, useful_visits = recommend_medication(nsp_batch, data_val, sample_rate=10,
                                                      similarity_threshold=0.5)
        result = model(nsp_batch, useful_visits, mode='pretrain_nsp')
...
def random_mask_word(seq, vocab, mask_prob=0.15):
    ...
    for i, _ in enumerate(seq): # 遍历输入序列中的每个 token
        prob = random.random() # 生成一个 0 到 1 之间的随机数
...
def main(args):
    ...
    model = RPNet(args, voc_size, ddi_adj)
    logging.info(model)
    ...
    for epoch in range(EPOCH):
        ...
        for step, patient in tqdm(enumerate(data_train), ncols=60, desc="finetune", total=len(data_train)):  # every patient
            patient, useful_visits = recommend_medication(patient, data_pretrain, sample_rate=10, similarity_threshold=0.5)
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_mask", total=len(data_train)):
            ...
            masked_batch, useful_visits = recommend_medication(masked_batch, data_train, sample_rate=10,
                                                            similarity_threshold=0.5)
            result = model(masked_batch, useful_visits, mode='pretrain_mask')
...
def main_nsp(args, model, optimizer, writer, data, data_train, data_val, device, save_dir, log_save_id):
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_nsp", total=len(data_train)):  # every patient
            ...
            nsp_batch, useful_visits = recommend_medication(nsp_batch, data_train, sample_rate=10,similarity_threshold=0.5)
            result = model(nsp_batch, useful_visits,mode='pretrain_nsp') # 返回值是一个 tensor([0.6069, 0.5388]
            loss = F.cross_entropy(result, torch.tensor(nsp_target, device=device, dtype=torch.float32))
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
