...
def get_args():
    ...
    parser.add_argument('--early_stop', type=int, default=7, help='early stop after this many epochs without improvement')
    parser.add_argument('-t', '--test', action='store_true', help="test mode")
    # pretrain的模型参数也是利用log_dir_prefix来确定是哪个log里的模型
    parser.add_argument('-l', '--log_dir_prefix', type=str, default='D:\code\RAREMed-main\log\mimic-iii\RAREMed\log0_\Epoch_10_JA_0.5387_DDI_0.07617.model', help='log dir prefix like "log0", for model test')
    parser.add_argument('-p', '--pretrain_prefix', type=str, default='D:\code\RAREMed-main\log\mimic-iii\RAREMed\log0_\saved.pretrained_model', help='log dir prefix like "log0", for finetune')
    # parser.add_argument('-l', '--log_dir_prefix', type=str,
    #                     default='',
    #                     help='log dir prefix like "log0", for model test')
    # parser.add_argument('-p', '--pretrain_prefix', type=str,
    #                     default='',
...
# [诊断代码列表, 手术代码列表, 药物代码列表, 就诊得分(数值)]
# 第一条记录: [
#     [[0,1,2,3,4,5,6,7], [0,1,2], [0,1,2,...,15], 10.15547877591313],
#     [[8,9,10,7], [3,4,1], [0,1,2,...,18], 0.9671884548488696]
# ]
def jaccard_similarity(set1, set2):
    """计算两个集合的Jaccard相似性"""
    # 如果 set1 是标量值，则将其转换为单元素列表
    if not isinstance(set1, (list, tuple)):
        set1 = [set1]
    # 如果 set2 是标量值，则将其转换为单元素列表
    if not isinstance(set2, (list, tuple)):
        set2 = [set2]

    # 将列表或元组转换为集合
    set1 = set(tuple(x) if isinstance(x, (list, tuple)) else x for x in set1)
    set2 = set(tuple(x) if isinstance(x, (list, tuple)) else x for x in set2)
...
# def recommend_medication(batch_data, data, sample_rate=300, similarity_threshold=0.0075):
#     sample_rate=int(len(data)*0.3)
#     # print("*********", data[:10])
#     nsp_batch = []
#     nsp_target = []
#     useful_visit = []
#     for visit in batch_data:
#         # print(f"Visit: {visit}")  # 调试信息
#         nsp_batch.append(visit)
#         for i in range(sample_rate):
#             random_visit = random.choice(data)
#             # print(f"Random Visit: {random_visit}")  # 调试信息
#             if len(random_visit) < 2 or len(visit) < 2:
#                 continue
#             while random_visit == visit:  # 放宽条件
#                 random_visit = random.choice(data)
#             # print("6666666666",random_visit[0],)
#             # print("7777777",visit[0])
...
#             if diagnosis_similarity >= similarity_threshold or procedure_similarity >= similarity_threshold:
...
def recommend_medication(batch_data, preprocessed_pool,
    ...
    直接整合到训练循环的推荐函数
    Args:
        batch_data: 当前患者的所有就诊记录 [visit1, visit2,...]
        preprocessed_pool: 预处理好的数据池 {
            'score_bins': {score: [visits]},
            'unique_scores': [sorted_scores]
        }
        similarity_threshold: 相似度阈值
        max_candidates: 每个就诊最多返回的相似记录数
        score_range: 得分搜索范围(±值)
    Returns:
        tuple: (
            原样返回的batch_data,
            相似就诊列表 [[visit1_similars], [visit2_similars],...]
        )
    """
    ...
    for visit in batch_data:
        ...
        # 1. 确定目标得分范围  通过预分类+范围查询显著提高了检索效率
        if len(visit) > 3:  # 有得分的情况
            target_score = visit[-1]
        ...
        for score in neighbor_scores:
            candidates.extend(score_bins.get(score, []))

        # 3. 筛选相似记录
        for cand in random.sample(candidates, min(max_candidates * 3, len(candidates))):
            ...
            if diag_sim >= similarity_threshold or proc_sim >= similarity_threshold:
                useful_visits.append(cand)
                if len(useful_visits) >= max_candidates:
                    break

        all_useful_visits.append(useful_visits)

    if all_useful_visits:
        return batch_data, all_useful_visits[0]
        # print("$$$$$$$$",len(useful_visit))
    else:
        return batch_data, []


# def recommend_medication(batch_data, data, sample_rate=1000, similarity_threshold=0.05):
...
#             # 确保 random_visit 和 visit 的长度足够
#             if len(random_visit) < 2 or len(visit) < 2:
#                 continue  # 跳过长度不足的记录
#             while random_visit[0] == visit[0] and random_visit[1] == visit[1]:
#                 random_visit = random.choice(data)
#             diagnosis_similarity = jaccard_similarity(random_visit[0], visit[0])
#             procedure_similarity = jaccard_similarity(random_visit[1], visit[1])
#             if diagnosis_similarity >= similarity_threshold and procedure_similarity >= similarity_threshold:
#                 useful_visit.append(random_visit)
#     print('11111111111111111',useful_visit)
#     return nsp_batch, useful_visit


# evaluate
@torch.no_grad()
def evaluator(args, model, data_val, voc_size, epoch, preprocessed_pool,
    ...
    for patient in tqdm(data_val, ncols=60, total=len(data_val), desc=f"Evaluation"):
        ...
        patient, useful_visits = recommend_medication(patient, preprocessed_pool, similarity_threshold=0.05, max_candidates=10,
        score_range=0.5)
        results, loss_ddi = model(patient,useful_visits)
    ...
    logging.info(f'''Epoch {epoch:03d}, Jaccard: {np.mean(ja):.4}, DDI Rate: {ddi_rate:.4}, PRAUC: {np.mean(prauc):.4}, AVG_F1: {np.mean(avg_f1):.4}, AVG_PRC: {np.mean(avg_p):.4f}, AVG_RECALL: {np.mean(avg_r):.4f}, AVG_MED: {med_cnt / visit_cnt:.4f}''')
...
def evaluator_mask(model, data_val, voc_size, epoch, device, mode='pretrain'):
    ...
    for batch in tqdm(data_val, ncols=60, desc=mode, total=len_val):
        ...
        # batch, useful_visits = recommend_medication(batch, data_val, sample_rate=10,
        #                                               similarity_threshold=0.5)
        result = model(batch, None, mode)
...
def evaluator_nsp(model, data_val, data, epoch, device, mode='pretrain_nsp'):
    ...
    for batch_data in tqdm(data_val, ncols=60, desc=mode, total=len(data_val)):
        ...
        # nsp_batch, useful_visits = recommend_medication(nsp_batch, None, similarity_threshold=0.2, max_candidates=5,
        # score_range=0.4)
        result = model(nsp_batch, None, mode='pretrain_nsp')
...
# 对输入序列进行随机掩码操作，用于 Masked Language Model (MLM)。通过预测被掩码的诊断或手术代码，模型能够学习到这些代码之间的上下文关系。
def random_mask_word(seq, vocab, mask_prob=0.15):
    mask_idx = vocab.word2idx['[MASK]'] # 获取 [MASK] 标记在词汇表中的索引
    for i, _ in enumerate(seq): # 遍历输入序列中的每个 token
        prob = random.random() # 生成一个 0 到 1 之间的随 机数
        # mask token with 15% probability # 如果随机数小于掩码概率，则对当前 token 进行掩码操作
        if prob < mask_prob: # 这个比例或许可以改一下
            prob /= mask_prob  # 将随机数归一化到 0 到 1 之间
            # 80% randomly change token to mask token   # 80% 的概率：将当前 token 替换为 [MASK] 标记
            if prob < 0.8:
                seq[i] = mask_idx
            # 10% randomly change token to random token # 从词汇表中随机选择一个 token
            elif prob < 0.9:
                seq[i] = random.choice(list(vocab.word2idx.items()))[1]
            else:
                pass
        else: # 10% 的概率：保持当前 token 不变
            pass
    return seq
...
#  Next Sentence Prediction (NSP) 输入两个序列（如两次就诊记录），模型需要判断它们是否是连续的。通过 NSP 任务，模型能够学习到就诊记录之间的时间关系。
#  生成 NSP 任务的训练数据，包括正样本和负样本对。
#  正样本是连续的就诊记录，负样本是随机组合的就诊记录
def nsp_batch_data(batch_data, data, neg_sample_rate=1):
    nsp_batch = []  # 初始化存储样本对的列表
    nsp_target = []  # 初始化存储样本对标签的列表
    for visit in batch_data:  # 遍历当前批次中的每个就诊记录
        nsp_batch.append(visit)  # 将当前就诊记录作为正样本添加到 nsp_batch 中
        nsp_target.append(1)  # 将标签 1（表示正样本）添加到 nsp_target 中
        for i in range(neg_sample_rate):  # 为每个正样本生成 neg_sample_rate 个负样本
            neg_visit = random.choice(data)  # 从完整数据集中随机选择一个就诊记录作为负样本
            while neg_visit[1] == visit[1]:  # 确保负样本的手术代码与正样本的手术代码不同
                neg_visit = random.choice(data)
            if random.random() < 0.5:  # 以 50% 的概率生成一种负样本
                nsp_batch.append([visit[0], neg_visit[1]]) # 使用正样本的诊断代码和负样本的手术代码
                nsp_target.append(0)  # 将标签 0（表示负样本）添加到 nsp_target 中
            else:  # 以 50% 的概率生成另一种负样本
                nsp_batch.append([neg_visit[0], visit[1]]) # 使用负样本的诊断代码和正样本的手术代码
                nsp_target.append(0) # 将标签 0（表示负样本）添加到 nsp_target 中
        # print("5555555555",len(nsp_batch), nsp_target)
    return nsp_batch, nsp_target # nsp_batch要扔到模型里计算返回值作为预训练的一个子任务的预测值 nsp_target作为子任务的真实值  [1, 0]
...
def main(args):
    ...
    data_pool_path = f'./data/output/{args.dataset}' + '/preprocessed_pool.pkl'
    
    device = torch.device('cuda:{}'.format(args.cuda))

    data = dill.load(open(data_path, 'rb'))
    voc = dill.load(open(voc_path, 'rb'))
    preprocessed_pool =dill.load(open(data_pool_path, 'rb'))
    ...
    data_train = batchify(data_train, args.batch_size)#[:100]
    data_val = batchify(data_val, args.batch_size)#[:10]
    ...
    for epoch in range(EPOCH):
        ...
        for step, patient in tqdm(enumerate(data_train), ncols=60, desc="finetune", total=len(data_train)):  # every patient
            patient, useful_visits = recommend_medication(patient, preprocessed_pool, similarity_threshold=0.05, max_candidates=10,
        score_range=0.5)
            # print("有没有useful_visits",useful_visits)
            result, loss_ddi = model(patient,useful_visits)
        ...
        loss_val_bce, loss_val_multi, loss_val_ddi, loss_val_all = evaluator(
            args, model, data_val, voc_size, epoch, preprocessed_pool, ddi_adj_path, device)
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_mask", total=len(data_train)):
            ...
            if args.mask_prob > 0:
                ...
                masked_batch = batch
            # masked_batch, useful_visits = recommend_medication(masked_batch, data_train, sample_rate=10,
            #                                                 similarity_threshold=0.5)
            result = model(masked_batch, None, mode='pretrain_mask')
...
def main_nsp(args, model, optimizer, writer, data, data_train, data_val, device, save_dir, log_save_id):
    ...
    for epoch in range(EPOCH):
        ...
        for batch in tqdm(data_train, ncols=60, desc="pretrain_nsp", total=len(data_train)):  # every patient
            ...
            # nsp_batch, useful_visits = recommend_medication(nsp_batch, data_train, sample_rate=10)
            result = model(nsp_batch, None,mode='pretrain_nsp') # 返回值是一个 tensor([0.6069, 0.5388]
            loss = F.cross_entropy(result, torch.tensor(nsp_target, device=device, dtype=torch.float32))
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
...
def loss_func(voc_size, patient, results, device):
    ...
    for idx, adm in enumerate(patient):
        ...
        loss_multi_target_tensor = torch.tensor(loss_multi_target, dtype=torch.long, device=device)
        loss_multi = F.multilabel_margin_loss(F.sigmoid(result), loss_multi_target_tensor)
